...
-module(rabbit_direct_presence_exchange).
-include_lib("rabbit_common/include/rabbit.hrl").
-include_lib("rabbit_common/include/rabbit_framing.hrl").

-define(EXCHANGE_TYPE_BIN, <<"x-direct-presence">>).
-define(LISTENER_KEY, <<"">>).

-behaviour(rabbit_exchange_type).

-rabbit_boot_step({?MODULE,
                   [{description, "exchange type x-presence with direct send allowed"},
...
description() ->
    [{name, ?EXCHANGE_TYPE_BIN},
     {description, <<"Experimental Presence exchange with direct message delivery allowed">>}].
...
add_binding(none, #exchange{name = XName}, #binding{key = ?LISTENER_KEY,
    ...
    case rabbit_misc:table_lookup(ArgsTable, <<"x-presence-exchange-summary">>) of
        ...
        _             -> rabbit_presence_exchange:announce_initial_bindings(XName, Dest)
    end,
    ok;
