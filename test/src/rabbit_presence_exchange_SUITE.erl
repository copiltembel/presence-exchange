-module(rabbit_presence_exchange_SUITE).
-include_lib("eunit/include/eunit.hrl").
-include_lib("rabbit_common/include/rabbit.hrl").
-include_lib("rabbit_common/include/rabbit_framing.hrl").

-export([all_tests/0]).
-import(rabbit_presence_exchange, [description/0, serialize_event/0]).

%% Ensure description returns what we configured
description_test() ->
    ?assertEqual(
       rabbit_presence_exchange:description(),
       [{name, <<"x-presence">>},
...
serialise_events_test() ->
    ?assertEqual(rabbit_presence_exchange:serialise_events(), false).

% announce_initial_bindings_no_binding_test() ->
%     rabbit_presence_exchange:announce_initial_bindings([]),
%     ok.
...
add_binding_skip_initial_announcement_test() ->
    ...
    AddBinding = rabbit_presence_exchange:add_binding(Transaction, Exchange, Binding),
...
add_binding_test() ->
    ...
    meck:new(rabbit_presence_exchange, [passthrough]),
    meck:expect(rabbit_presence_exchange, announce_initial_bindings,
                fun(XName, Dest) ->
    ...
    %% Then I see it should just return ok without calling the
    %% function announce_initial_bindings()
    ?assert(meck:validate(rabbit_presence_exchange)),
    ?assertEqual(AddBinding, ok),

    meck:unload(rabbit_presence_exchange),
    ok. %% Have a nice day!
